...
DECLARE @nome_das_lojas VARCHAR(200)

SET @nome_das_lojas =''

SELECT 
	@nome_das_lojas =  @nome_das_lojas + StoreName + ' e '
FROM
	DimStore 
WHERE CloseDate LIKE '%2008%'

PRINT 'As lojas fechadas no ano de 2008 foram: ' + LEFT(@nome_das_lojas, DATALENGTH(@nome_das_lojas)-2)